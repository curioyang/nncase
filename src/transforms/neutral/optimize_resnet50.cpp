...
bool fold_binary_into_deq_transform::on_try_match(node &node, transform_context &context)
{
    // constant *mul_const, *add_const;
    if (auto b_mul = node_cast<binary>(node))
    {
        if (auto b_max = try_get_direct_parent<binary>(*b_mul, 0))
        {
            if (auto b_add = try_get_direct_child<binary>(*b_mul))
            {
                if (b_mul->binary_op() == binary_mul && b_max->binary_op() == binary_max && b_add->binary_op() == binary_add)
                {
                    if (b_add->fused_activation() != value_range<float>::full())
                        return false;
                    if (auto mul_const = try_get_direct_parent<constant>(*b_mul, 1))
                    {
                        context.inputs.emplace_back(&b_mul->input_a());
...
void fold_binary_into_deq_transform::process([[maybe_unused]] transform_context &context)
{
    auto &output = *context.inputs[0]->connection();
    auto inputs = context.outputs[0]->connections();
    auto &const_mul = static_cast<constant &>(*context.matched_nodes[0]);
